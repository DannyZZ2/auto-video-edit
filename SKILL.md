...
- Use the bundled style fidelity assets in `references/` and `templates/remotion-overlay-kit/` whenever generating the built-in overlay styles. These files are part of the skill, not optional inspiration.
...
- 生成内置 overlay 风格时，必须使用 `references/` 和 `templates/remotion-overlay-kit/` 中随 skill 打包的风格保真资产。这些文件是 skill 的组成部分，不是可选灵感。

## Style Fidelity Assets / 风格保真资产

The built-in style family is preserved through both specification files and reusable Remotion templates. Before designing or implementing any built-in style, read the relevant files below and cite their constraints in the packaging plan.

内置风格族通过“规范文件 + 可复用 Remotion 模板”一起保真。设计或实现任何内置风格前，必须读取下面相关文件，并在包装方案里引用实际采用的约束。

Required style references:

必须读取的风格参考：

- `references/card-style-library.md`: the active 5-style family and when each variant is allowed.
- `references/dark-diagnostic-hud-style-system.md`: complete Dark Diagnostic HUD tokens, layout grammar, component rules, motion language, and failure checks.
...
- For every built-in style, preserve its own `tokens.json.geometry` contract: card radius, inner/chip radius, border width, border style, and forbidden drift shapes. The shared rule is to read the selected style's geometry; the actual geometry values are different for each style.
- Apply Dark Diagnostic HUD's continuous rounded rectangle rule only to `dark-diagnostic-hud`: outer radius `16px`, inner/icon radius `12px`, and `2px` semantic border. Do not apply that shape to Signal Desk, Precision HUD, Diagnostic Glass, or Terminal Agent unless their own tokens say so.
...
- 每一种内置风格都必须保留自己的 `tokens.json.geometry` 几何契约：卡片圆角、内部/chip 圆角、描边宽度、描边样式和禁用跑偏形态。共用规则只是“读取所选风格的几何”，具体几何数值必须逐风格独立。
- `Dark Diagnostic HUD / 暗色诊断 HUD` 的连续圆角矩形规则只适用于 `dark-diagnostic-hud`：外层圆角 `16px`，内部/图标圆角 `12px`，语义描边 `2px`。不要把这套形态套到 Signal Desk、Precision HUD、Diagnostic Glass 或 Terminal Agent 上，除非它们自己的 tokens 这样定义。
...
风格资产来源：references/dark-diagnostic-hud-style-system.md + references/card-style-library.md。
...
风格资产来源：references/dark-diagnostic-hud-style-system.md + references/visual-quality-system.md。
...
- Do not use a generic "HUD/card" shape across all styles. Resolve the selected style and apply only that style's `tokens.json.geometry`.
- Do not mix geometry between styles. For example, Dark Diagnostic cards must not become checkpoint frames, glass cards must not become precision HUD frames, Signal Desk popups must not become large HUD boxes, and Terminal Agent cards must not become native OS window clones.
...
- 不要对所有风格套用泛化的 “HUD/card” 形态。先解析所选风格，然后只应用该风格自己的 `tokens.json.geometry`。
- 不要混用不同风格的几何。比如 Dark Diagnostic 卡片不要变成 checkpoint 外框，玻璃卡片不要变成精密 HUD 框，Signal Desk 弹窗不要变成大 HUD 框，Terminal Agent 卡片不要变成原生系统窗口仿制。
