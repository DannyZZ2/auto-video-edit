...
- Dark Diagnostic 主面板：暗色半透明底、连续圆角矩形、2px 语义描边、弱外发光、内阴影。外圆角 `16px`，内部图标/小框圆角 `12px`。其他风格必须按自己的几何契约执行，不套用这组数值。
...
- 不把卡片做成纯色扁平矩形。
- 不把一种风格的几何套到另一种风格上。Dark Diagnostic 不做 checkpoint / corner bracket，Signal Desk 不做大 HUD 框，Precision 不做玻璃泡泡，Glass 不做不透明 HUD 框，Terminal 不做原生系统窗口仿制。
- 不把完整口播句子作为大字卡片。
