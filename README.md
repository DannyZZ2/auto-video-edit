...
2. If ElevenLabs is selected, ask upload consent first, then ask for API-key availability.
3. Ask whether the user needs editing.
4. If editing is needed, ask for the source video folder first, then ask normal edit or fine-cut as a separate follow-up.
5. If editing is not needed, ask for the already-edited video.
6. Generate or normalize SRT, transcript, timestamp, edit-decision, keyword-cue, current-project asset, and gesture-cue data into one packaging timing bundle.
...
10. If pointing, dragging, swiping, circling, or line-drawing gestures are visible near a cue, use the gesture position as the preferred image/animation anchor.
...
2. 如果选择 ElevenLabs，先问是否同意上传音频，再单独询问 API key 是否可用。
3. 再问用户是否需要剪辑。
4. 如果需要剪辑，先让用户提供源视频文件夹，再单独询问默认剪辑或精剪。
5. 如果不需要剪辑，让用户提供已经剪辑好的视频。
6. 将 SRT、转写文本、时间戳、剪辑决策、关键词 cue、当前项目素材和手势 cue 统一整理为包装时间包。
...
10. 如果画面中在 cue 附近有指、拖、划、圈选或画线手势，优先使用手势位置作为图片/动画锚点。
